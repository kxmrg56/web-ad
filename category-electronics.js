...
async function loadProductsFromDatabase() {
    try {
        // 注意：这里应该是electronics（手机数码）
        const response = await fetch('/shopping_web_war_exploded/api/products?category=electronics');
        ...
        console.log('成功加载手机数码商品:', categoryProducts.length);
        ...
        categoryProducts = [
            {
                id: 201,
                name: '索尼 85英寸4K电视',
                description: 'XR认知芯片，全阵列背光，杜比视界',
                price: 12999,
                originalPrice: 14999,
                stock: 8,
                imageUrl: 'https://via.placeholder.com/300x200?text=索尼电视'
            }
        ];
...
function renderProducts() {
    ...
    <div class="product-image">
        <img src="${product.imageUrl}" 
             alt="${product.name}" 
             class="product-img"
             onerror="this.src='https://via.placeholder.com/300x200?text=${encodeURIComponent(product.name.substring(0,10))}';">
    </div>
