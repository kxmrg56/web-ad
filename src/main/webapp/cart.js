...
// 在 ShoppingCart 类中添加这个方法
    async deleteSelected() {
        const selectedItems = this.getSelectedItems();

        if (selectedItems.length === 0) {
            this.showNotification('请至少选择一件商品', 'error');
            return;
        }
        ...
        try {
            ...
            const deletePromises = selectedItems.map(item =>
                fetch(`${this.baseUrl}/${item.productId}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            this.showNotification(`已删除 ${selectedItems.length} 件商品`, 'success');
...
async function checkout() {
    ...
    if (!confirm(`确定要结算 ${cart.getTotalItems()} 件商品吗？`)) {
        return;
    }

    try {
        const response = await fetch('/shopping_web_war_exploded/api/cart/checkout', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
