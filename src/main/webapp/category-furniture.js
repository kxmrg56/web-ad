...
async function loadProductsFromDatabase() {
    try {
        // 调用API获取商品数据 - 注意：category=furniture
        const response = await fetch('/shopping_web_war_exploded/api/products?category=furniture');
        ...
        console.log('成功从数据库加载', categoryProducts.length, '个生活家具商品');

    } catch (error) {
        console.error('从数据库加载商品失败:', error);

        // 如果API失败，保留原有商品数据作为备用
        categoryProducts = [
            {
                id: 301,
                name: '真皮沙发组合',
                description: '头层牛皮，舒适坐感，现代简约设计，客厅家具首选',
                price: 8999,
                originalPrice: 11999,
                stock: 15
            }
